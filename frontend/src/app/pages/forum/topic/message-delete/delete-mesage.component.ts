...
export class DeleteMessageComponent {
  ...
  deleteMessage() {
    this.topicService.deleteMessage(this.message.id).subscribe(() => {
      this.updateArray();
      this.modal.close();
  ...
  private updateArray(): void {
    this.updated.emit(this.messages.filter(r => r !== this.message));
  }

  private handleErrorDeleteUser(error) {
    switch (error.status) {
      case 500:
        this.toastr.error("Can't delete message", 'Error');
    }
  }
}
